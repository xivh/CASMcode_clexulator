...
- :func:`~libcasm.clexulator.ConfigDoFValues.local_dof_values`: Local continuous DoF values, as an two-dimensional floating-point arrays, accessed by DoF type, with one column of values for each site in the supercell.
- :func:`~libcasm.clexulator.ConfigDoFValues.global_dof_values`: Global continuous DoF values, as an one-dimensional floating-point arrays, accessed by DoF type.

For CASM standard DoF types, this module provides methods for constructing properly sized :class:`~libcasm.clexulator.ConfigDoFValues` instances, and converting between two useful choices of basis as described below. See the CASM `Degrees of Freedom (DoF) and Properties`_ documentation for the full list of supported DoF types and their definitions. Some examples include:
...
- :func:`~libcasm.clexulator.ConfigDoFValues.local_dof_values`: Local continuous DoF values are accesible as a `numpy.ndarray[numpy.float64[matrix_dim, n_sites]]`, where `n_sites` is the number of sites in the supercell, and `matrix_dim` varies by DoF type and choice of basis. A DoF type string (i.e. `"disp"` for displacements) is used as a key to access the values array.
  ...
  - Values can be accessed using indices `[dof_component_index, linear_site_index]`:
    ...
    - Rows of :func:`~libcasm.clexulator.ConfigDoFValues.local_dof_values` correspond to a DoF basis vector.

- :func:`~libcasm.clexulator.ConfigDoFValues.global_dof_values`: Global continuous DoF values, accesible by DoF type as a `numpy.ndarray[numpy.float64[vector_dim, 1]]`, where `vector_dim` varies by DoF type and choice of basis. A DoF type string (i.e. `"GLstrain"` for strain represented using the Green-Lagrange strain metric) is used  as a key to access the values array.
  ...
  - Values can be accessed using index `[dof_component_index]`:
    ...
    - Elements of :func:`~libcasm.clexulator.ConfigDoFValues.global_dof_values` correspond to a DoF basis vector.


Choice of basis
---------------

There are two supported choices of basis for the continuous DoF values in :class:`~libcasm.clexulator.ConfigDoFValues`:
...
- The "prim DoF basis", which is specified by the :class:`~libcasm.xtal.DoFSetBasis` used to define the space of allowed DoF in a :class:`~libcasm.xtal.Prim`
  ...
  - Allows specifiying DoF values, including those that have been constrained to a a subspace of the full standard DoF space (i.e. two-dimensional displacements, or only isotropic strains) using independent variables
  - Used as the input to cluster expansion and order parameter calculators


When using the standard DoF basis:

- The `matrix_dim` of local DoF and `vector_dim` of global DoF is the standard DoF dimension (i.e. 3 for displacements, 6 for strain), and DoF components are ordered using the CASM standard definitions. See the CASM `Degrees of Freedom (DoF) and Properties`_ documentation for definitions.

When using the prim DoF basis:

- The `matrix_dim` of local DoF is the maximum dimension of the basis for that DoF type on any sublattice.
...
There are factory functions to construct properly sized and zero-valued :class:`~libcasm.clexulator.ConfigDoFValues` instances for each:
...
Conversions between the two choices of basis can performed using:
...
To support serialization, :class:`~libcasm.clexulator.ConfigDoFValues` can be converted to and from representations using plain old Python data types (``list``, ``dict``, ``int``, ``float``, ``bool``, ``str``) using:

- :class:`~libcasm.clexulator.ConfigDoFValues.to_dict`
- :class:`~libcasm.clexulator.ConfigDoFValues.from_dict`

Either ``json`` (for long term storage and portability) or ``pickle`` (for fast local storage) can be used for serialization / deserialization.
...
Conversions to and from other data structures, such as conversion to :class:`~libcasm.xtal.Structure` and mapping from :class:`~libcasm.xtal.Structure` are outside the scope of this module. See CASMcode_configuration and CASMcode_mapping for those type of conversions.
